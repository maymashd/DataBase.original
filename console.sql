...
ALTER TABLE Movie add constraint a1 primary key(mID);
ALTER TABLE MOVIE ADD CONSTRAINT a2 unique (title,year);
ALTER TABLE Reviewer add constraint a3 primary key(rID);
ALTER TABLE Rating ADD CONSTRAINT a4 unique (rID,mID,ratingDate);
ALTER TABLE Reviewer ALTER COLUMN name SET NOT null ;
ALTER TABLE Rating ALTER COLUMN stars SET NOT null ;
ALTER TABLE Movie ADD CONSTRAINT A5 CHECK(year>1900);
ALTER TABLE Rating ADD CONSTRAINT A6 CHECK(stars BETWEEN 1 AND 5);
ALTER TABLE Rating ADD CONSTRAINT A7 CHECK(ratingDate > TO_DATE('2000-01-01','YYYY-MM-DD'));
